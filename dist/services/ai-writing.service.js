...
exports.generateWritingAssistance = void 0;
...
const reserveDailyUsage = async (userId) => {
  ...
  `);
    ...
    if (!count) {
        throw new errors_1.AppError(`Limit generate AI harian sudah tercapai (${dailyLimit} kali per hari).`, 429, "AI_DAILY_LIMIT_REACHED");
    }
    return {
        date: today,
        used: count,
...
const releaseDailyUsage = async (userId, usageDate) => {
...
const normalizeOpenRouterError = (status, data) => {
...
const generateWritingAssistance = async (userId, payload) => {
    const apiKey = process.env.OPENROUTER_API_KEY || process.env.VITE_OPENROUTER_API_KEY;
    if (!apiKey) {
        throw new errors_1.AppError("OPENROUTER_API_KEY belum diatur. Tambahkan API key OpenRouter di file .env.", 500, "OPENROUTER_KEY_MISSING");
    }
    ...
    try {
        ...
        if (!response.ok) {
            throw new errors_1.AppError(normalizeOpenRouterError(response.status, data), response.status, "OPENROUTER_ERROR");
        }
        const text = data.choices?.[0]?.message?.content?.trim() || "";
        if (!text) {
            throw new errors_1.AppError("AI tidak mengembalikan teks yang bisa dipakai.", 502, "AI_EMPTY_RESPONSE");
        }
        return {
...
exports.generateWritingAssistance = generateWritingAssistance;
