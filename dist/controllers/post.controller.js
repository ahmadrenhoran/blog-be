...
const createPost = async (req, res, next) => {
    try {
        ...
        const newPost = await postService.createPost(userId, title, content, cover_image);
        response_1.ApiResponse.success(res, newPost, "Successfully created a new post", 200);
    }
    catch (error) {
        next(error);
    }
...
const getPostById = async (req, res, next) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
        const newPost = await postService.getPostById(parseInt(id), userId);
        response_1.ApiResponse.success(res, newPost, "Successfully created a new post", 200);
    }
    catch (error) {
        next(error);
...
const deletePostById = async (req, res, next) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
        const newPost = await postService.deletePostById(parseInt(id), userId);
        response_1.ApiResponse.success(res, newPost, "Successfully created a new post", 200);
    }
    catch (error) {
        next(error);
